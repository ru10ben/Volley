...
//		addHeader("Accept-Encoding", "identity");
	...
	public byte[] handleResponse(HttpResponse response, ResponseDelivery delivery) throws IOException, ServerError {
		...
		boolean isSupportRange = HttpHeaderParser.isSupportRange(response);
		if (isSupportRange) {
			...
			String realRangeValue = HttpHeaderParser.getHeader(response, "Content-Range");
	 ...
	 */
	@Override
	public Request<Void> setCacheExpireTime(TimeUnit timeUnit, int amount) {
	    return this;
	}
